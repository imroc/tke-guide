...
Re-tested 2026-09 on an Overlay (VPC-CNI) cluster (cilium **1.20.1** + cilium CLI v0.20.0 + TencentOS Server 4.4 / kernel 6.6.88) with the new scheme (`sysctlfix.enabled=false` + the `cilium-sysctl-override` DaemonSet pinning rp_filter=0 on all node interfaces; see [sysctlfix and rp_filter Deep Dive](./sysctlfix.md)):
