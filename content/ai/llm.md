...
TKE 有**普通节点**、**原生节点**和**超级节点**三种节点类型，以下是它们的对比：

| 节点类型 | 扩容速度                     | 维护难度                                                                                       | GPU 资源丰富度                                                                                                                                             |
| -------- | ---------------------------- | ---------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 普通节点 | 需扩容节点时较慢             | 较大                                                                                           | 最全（腾讯云上所有GPU型号都支持）                                                                                                                          |
| 原生节点 | 需扩容节点时较慢             | 一般（运维负担更低，参考[原生节点概述](https://cloud.tencent.com/document/product/457/78197)） | 中等                                                                                                                                                       |
| 超级节点 | 快（只管扩 Pod，无需扩节点） | 极简（是虚拟的节点，每个 Pod 都是独立的轻量虚拟机，无需维护节点）                              | 较少（通过 Pod 注解指定型号，可用列表参考[这里](https://cloud.tencent.com/document/product/457/39808#gpu-.E8.A7.84.E6.A0.BC)，主要是V100、T4、A10 这几种） |
...
- vLLM 官方支持通过 Ray 实现多机分布式部署，参考 [Running vLLM on multiple nodes](https://docs.vllm.ai/en/latest/serving/distributed_serving.html)。
