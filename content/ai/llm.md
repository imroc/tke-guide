...
| -------- | ---------------------------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| 普通节点 | 需扩容节点时较慢             | 较大                                                                                           | 最全（所有型号都支持）                                                                                                         |
| 原生节点 | 需扩容节点时较慢             | 一般（运维负担更低，参考[原生节点概述](https://cloud.tencent.com/document/product/457/78197)） | 中等                                                                                                                           |
| 超级节点 | 快（只管扩 Pod，无需扩节点） | 极简（是虚拟的节点，每个 Pod 都是独立的轻量虚拟机，无需维护节点）                              | 最少（通过 Pod 注解指定型号，可用列表参考[这里](https://cloud.tencent.com/document/product/457/39808#gpu-.E8.A7.84.E6.A0.BC)） |
