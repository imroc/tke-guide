...
- **引入 etcd**：flannel 另外也支持使用 etcd 来存储网段配置和 IP 分配信息，需引入额外的 etcd，需要准备好 etcd（自行部署或购买 [云原生etcd](https://cloud.tencent.com/document/product/457/58176?from=console_document_search) 实例），然后在安装 flannel 时配置好 etcd 相关参数（对应 chart 的 `flannel.args` 参数），flannel 参数参考 [flannel 配置文档](https://github.com/flannel-io/flannel/blob/master/Documentation/configuration.md#key-command-line-options)。
