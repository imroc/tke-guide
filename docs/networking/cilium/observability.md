...
169.254.0.71 是 CLS 的 API 地址的目标 IP，采集的日志会通过这个 IP 上报，如果没有指定 includeFilters，需加上这个 excludeFilters 避免将上报 CLS 日志的流量也作为 cilium 网络日志流记录下来，然后又采集这个日志，又上报，导致无限循环采集上报，即便在没有其他网络流量的情况下也会一直产生和采集新日志，造成不必要的开销。
