...
:::caution[VPC-CNI Native 模式：Pod 出公网必须启用 IP 伪装]

VPC-CNI Native 模式下，Pod IP 是从节点**辅助网卡**（secondary ENI）的 IP 池里分配的合法 VPC IP；而节点的 EIP（公网 IP）只绑在**主网卡**（primary ENI）上。所以 Pod 出公网时：

1. 数据包源 IP = Pod IP（辅助网卡上的 VPC IP）；
2. 内核根据路由表把数据包从对应的辅助网卡送出（不经过主网卡）；
3. 辅助网卡没有 EIP，包到了 VPC 边界后没有公网回包路径；
4. 结果：**即使节点绑了 EIP，Pod 也访问不了公网**。
...
- **GR / Overlay**：Pod IP 不是合法 VPC IP，cilium 默认就启用了 IP 伪装（`enableIPv4Masquerade=true`），Pod 出节点时已被 SNAT 成节点 IP，节点有 EIP 即可出公网；
- **Native**：Pod IP 是合法 VPC IP，cilium 默认关闭 IP 伪装（`enableIPv4Masquerade=false`），所以才有上述问题。
