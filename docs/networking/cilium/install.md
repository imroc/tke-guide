...
| IP 容量扩容          | ✅ 支持（新增 VPC-CNI 子网）            | ✅ 支持（追加 CIDR 到 CiliumPodIPPool；单节点用尽自动再分配） | ✅ 支持（追加 CIDR 到 CiliumPodIPPool；单节点用尽自动再分配）                                                                                                                          |
| 节点数量限制         | ✅ 无                                   | ✅ 无                                                         | ⚠️ 受 GR 集群的 ClusterCIDR 限制（GR 集群本身的限制）                                                                                                                                  |
...
| 适用场景             | 常规场景（推荐）                        | 需要 Gateway API、IP 资源紧张、纳管 IDC（推荐）               | 仅推荐已有 GR 集群的场景，不推荐为了装 cilium 新建 GR 集群                                                                                                                             |
...
安装前先确认版本（`kubectl -n kube-system get ds tke-eni-agent -o jsonpath='{.spec.template.spec.containers[0].image}'`），若低于 **v3.8.0**（建议 v3.8.3+，含存量 2000+ 路由表对账修复），请先通过 [组件管理](https://cloud.tencent.com/document/product/457/58234) 升级 eni 相关组件后再安装 cilium。v3.8.0 起 ENI 策略路由表已迁移到 `4000 + LinkIndex`，与 cilium 的 2004/2005 彻底错开。
...
operator:
  ...
  - key: "tke.cloud.tencent.com/uninitialized"
    operator: Exists
  # 避免调度到超级节点 (eklet)：cilium-operator 使用 hostNetwork，
  # 就绪探针 (127.0.0.1:9234) 在超级节点上探测不通，会导致容器持续重启。
  # 此 nodeAffinity 会与 chart 默认的 podAntiAffinity 合并，不影响多副本分散调度
  affinity:
    nodeAffinity:
...
ipam:
  # multi-pool 支持单节点动态分配多个 PodCIDR，突破 cluster-pool 的单节点 Pod 数量上限
  # 运行时可通过修改 CiliumPodIPPool 追加 CIDR 或调整 pre-allocation 数量
  mode: "multi-pool"
  operator:
    autoCreateCiliumPodIPPools:
      default:
        ipv4:
          ...
          # 运行时可在 CiliumPodIPPool 中追加新 CIDR 扩容集群可用 IP 总量
          cidrs:
          - "10.244.0.0/16"
          # 每节点子网掩码，/24 = 每节点 254 个 Pod IP
          # 单节点 IP 用尽时 cilium-agent 会自动从池中再分配一个 /24，无需重建节点
          maskSize: 24
...
如果希望拉取镜像具备更高的可用性，可 [使用 TCR 托管 Cilium 镜像](./appendix/tcr.md) 将 cilium 依赖镜像同步到自己的 [TCR 镜像仓库](https://cloud.tencent.com/product/tcr)，然后参考这里的依赖镜像替换的配置，将相应镜像再替换为自己同步后的镜像地址。
...
**[一键安装脚本](#一键安装脚本) 和上文 helm/values.yaml 示例已默认配置 `operator.affinity.nodeAffinity`，通过 `node.kubernetes.io/instance-type NotIn [eklet]` 显式禁止 cilium-operator 调度到超级节点**，正常按文档安装即可避免此问题。
...
| Overlay (VPC-CNI/GR)     | multi-pool       | ✅ 支持          |
...
**根因**：Overlay 模式使用 cilium 的 `ipam.mode=multi-pool` IPAM，cilium 从 CiliumPodIPPool 中动态分配 IP，不支持固定 IP。Pod 重建后 IP 会变化。

而 Native Routing (VPC-CNI) 模式使用 `ipam.mode=delegated-plugin`，IP 由 TKE VPC-CNI 插件分配，天然支持 [Pod 固定 IP](https://cloud.tencent.com/document/product/457/34994)——通过 TKE 的 StatefulSet 固定 IP、Pod 直接绑定 EIP 等能力均可正常使用。

如果业务依赖固定 IP（如：

- 有状态服务通过 IP 白名单互访
- 日志/监控 agent 按固定 IP 采集
- 需要给 Pod 直接绑定 EIP

），建议使用 **Native Routing 模式**。
...
- [配置 IP 伪装](./appendix/masquerading.md)
- [使用 TCR 托管 Cilium 镜像](./appendix/tcr.md)
