...
- 操作系统：推荐 **TencentOS 4** 或 **Ubuntu 24.04**。最低要求 Linux kernel >= 5.10（参考 [System Requirements](https://docs.cilium.io/en/stable/operations/system_requirements/)）。已验证的 OS 参考 [已验证的节点操作系统](#已验证的节点操作系统)。
...
- **[设计原理](#设计原理)**：「为什么这么做」类问题，理解后能更好判断特殊场景
- **[常见问题](#常见问题)**：「能不能 / 怎么做 / 出错怎么办」类问题
- **[参考信息](#参考信息)**：已验证的 OS 等参考数据

### 设计原理

#### 为什么 Native Routing 模式要加 local-router-ipv4 配置？

Native Routing 模式下，cilium 会在每台节点上创建 `cilium_host` 虚拟网卡并需要配置一个 IP 地址。由于 Native Routing 模式中 cilium 不负责 Pod IP 分配（IP 由 TKE CNI 分配），需要通过 `local-router-ipv4` 参数手动指定一个不会有冲突的 IP 地址。`169.254.32.16` 是 link-local 地址段，在 TKE 上不会与其它 IP 冲突，所以选择这个地址。

**Native Routing (VPC-CNI) 和 Native Routing (GR) 两种模式都需要此配置**：

- VPC-CNI 模式 Pod 直接挂在弹性网卡上，IP 由 VPC-CNI 分配，cilium 完全没有 Pod IP 来源信息。
- GR 模式 Pod IP 由 tke-bridge 从节点 PodCIDR 中分配，每节点 PodCIDR 不同。虽然 tke-bridge 的网关 IP（如 `<PodCIDR>.1`）已被节点路由占用，cilium 也无法直接复用；统一使用 link-local 的 `169.254.32.16` 既避免与 PodCIDR 冲突，也保持所有节点配置一致。

Overlay 模式不需要此配置，因为 cilium 自己管理 Pod IP 分配（cluster-pool IPAM），会自动为 `cilium_host` 分配 IP。
...
本文的安装脚本已默认创建 cilium 专属的 APF FlowSchema 和 PriorityLevelConfiguration，防止 cilium 的 list 请求影响其他组件。如果手动安装，建议也参照脚本配置。
