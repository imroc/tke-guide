...
cilium 1.18 起 Kafka L7 规则废弃、**1.20 已彻底移除**（策略 L7 规则仅剩 HTTP/DNS）。若从旧版本升级且存量策略中还有 kafka 规则，升级前必须删除，否则新版本下策略校验会失败。
...
TKE 使用 LogConfig 这个 CRD 配置日志采集规则，通过这种方式配置可以快捷配置索引，避免在控制台大量手动操作。完整 LogConfig 配置如下：
...
<FileBlock file="cilium/cls-logconfig.yaml" showLineNumbers showFileName />
...
| 信息丰富度 | 包含 Pod 名称、标签、Namespace、NetworkPolicy 裁决等 K8s 元数据，甚至还支持七层信息 | 主要包含 IP、端口、协议等网络层信息 |
