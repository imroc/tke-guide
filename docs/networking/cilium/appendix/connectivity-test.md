...
| #   | 用例                                      | 状态    | 说明                                         |
| --- | ----------------------------------------- | ------- | -------------------------------------------- |
| 55  | `pod-to-pod-encryption`                   | ⏭️ 跳过 | 需要 cilium < 1.18，本测试用 1.19.5          |
...
| 69  | `echo-ingress-from-client-tiered-wildcard-pass-l7` | ⏭️ 跳过 | 需要 cilium >= 1.20，本测试 1.19.5  |
...
2026-09 在 Overlay (VPC-CNI) 集群（cilium **1.20.1** + cilium CLI v0.20.0 + TencentOS Server 4.4 / kernel 6.6.88）按新方案（`sysctlfix.enabled=false` + `cilium-sysctl-override` DaemonSet 将全节点接口 rp_filter 统一固定为 0，详见 [sysctlfix 与 rp_filter 机制详解](./sysctlfix.md)）复测：
