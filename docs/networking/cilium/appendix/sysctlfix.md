...
### Native Routing (GR)：统一禁用（与 VPC-CNI 保持一致）

- **数据路径**：cilium chaining 接管全部 Pod 网络，没有 lxc 接口需要修复 rp_filter。
- **实测**：启用 sysctlfix 本身不会破坏网络。
- **决策原因**：为了与 Native Routing (VPC-CNI) 配置保持一致、避免 OS 默认 sysctl 配置变化引入边缘问题，**统一禁用** sysctlfix。
...
| Native Routing (GR)      | ❌ 统一禁用    | 与 VPC-CNI 保持一致，避免潜在边缘问题 |
