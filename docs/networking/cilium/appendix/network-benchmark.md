...
**关键认知**：iptables 的小规模 RPS 优势是 ClusterIP Service 在简单架构下的局部最优；一旦 Service 数量增长，iptables 的 O(n) 退化（见后续 Service Scale 章节）会迅速吞掉这部分优势。本测试的 Cilium Native 跑在 VPC-CNI cni-chaining 之下才被迫回退 Legacy Host Routing；BPF Host Routing 本身只要求 `kubeProxyReplacement=true` + `bpf.masquerade=true`，与路由模式（Native/Tunnel）无关。脱离 VPC-CNI cni-chaining 的纯 Cilium Native 集群也能启用 BPF Host Routing，Native RPS 会显著提升、不再有 ~18% 差距。
...
- **TCP_CRR**（每次新建连接）：Cilium 比 iptables 多 ~180-210 µs，因新连接的 SYN 包需走 BPF conntrack 创建 + Service 解析 + Legacy Host Routing 双重处理。
- **HTTP p99 @1000 QPS**：在真实业务负载下，三者**完全相同**（0.99 ms）—— 微秒级差异被应用层处理时间淹没。
- **Native vs Overlay**：跨节点延迟差异 < 10 µs。两者所走的 datapath 不同（Native 是 cni-chaining 下的 BPF + 内核栈双重处理，Overlay 是 BPF Host Routing + VXLAN encap/decap），但延迟代价数值上恰好相当。
...
| **RPS 长连接**               | iptables 比 Cilium 高 ~18%               | 92K vs 75K（双重处理开销）               |
...
iptables 在小规模 Service（千以下）的极限 RPS 测试中**绝对值领先 Cilium ~18%**——这是 VPC-CNI 链式 CNI 架构强制 Cilium 走 Legacy Host Routing 的必然结果，不是 Cilium 的缺陷。
