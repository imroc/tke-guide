...
三者均跑满 10+ Gbps 接近 VPC 突发带宽上限，吞吐量无差异。不同集群间的 ±4% 差异（10.88 vs 10.43）属于 VPC 带宽波动和不同物理节点间的网络路径差异，不代表方案差异。
...
| Via Svc c64 短连接       | **22,828** req/s | **10,721** req/s | —              | **-53%**           |
...
| keepalive 退化           | **+0.7%**    | **-0.0%**     | **-1.6%**      |

:::tip[O(1) vs O(n) Service 查找]

keepalive 场景下三者均无退化——因为 conntrack 缓存了连接状态，后续包无需再遍历规则链 / BPF map。O(n) vs O(1) 的差异只在**短连接**场景下可见（每个新 TCP 连接的 SYN 包必须遍历完整的 KUBE-SERVICES 链或查询 BPF map）：

- **iptables**：每条新连接顺序遍历 KUBE-SERVICES 链（O(n)），1000 Services = 6,142 条规则，引入可测量的延迟增长
- **Cilium eBPF**：BPF hash map O(1) 查找，无论 Service 数量多少查找速度恒定

在更大规模（如 5,000+ Services）下，iptables 短连接性能将按比例退化，而 Cilium 保持恒定。这是 Cilium 替换 kube-proxy 最核心的价值之一。
...
| **1000 Svc 退化（keepalive）** | 三者均无退化（conntrack 缓存） | < 2%（噪声）                                   |
...
2. **Cilium 的代价**：相比 iptables，Cilium 在极限 RPS 场景下有 ~12% 开销（VPC-CNI Native 模式下 Legacy Host Routing 双重处理），但换来了 NetworkPolicy、Hubble 可观测性、eBPF 安全策略等企业级能力。

3. **Cilium 的优势**：Service 查找为 O(1)——随着集群 Service 规模增长，iptables 短连接性能将线性退化（每条新连接需遍历 O(n) 规则链），而 Cilium 保持恒定。规模越大优势越明显。
