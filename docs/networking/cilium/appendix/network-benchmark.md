...
- **大规模 Service**：iptables 短连接性能随规则数**线性崩塌**（10000 Service × 10 Endpoint = 42 万条规则，短连接退化 42%），Cilium 几乎不退化（约 11-13%）。规模越大，天平越向 Cilium 倾斜。
...
| RPS 长连接（c64）         | 110,838    | 105,070       | 100,084        | iptables（+5~11%）           |
| RPS 短连接（c64，小规模） | 30,413     | 13,786        | 14,263         | iptables（+2.2x）            |
| TCP_RR p99                | 111 µs     | 96 µs         | 95 µs          | **Cilium（更低）**           |
| TCP_CRR p99               | 499 µs     | 558 µs        | 537 µs         | iptables（略低）             |
| HTTP p99 @1000 QPS        | 0.99 ms    | 0.99 ms       | 0.99 ms        | 平                           |
| 10000 svc 短连接退化      | **-42.3%** | **-13.2%**    | **-11.5%**     | **Cilium（差距随规模拉大）** |
| 10000 svc 规则/LB 条目    | 419,681    | 119,832       | 119,888        | -                            |
...
| 数据面组件内存 / 节点     | 278 MB     | 153 MB        | 165 MB         | 见[第六节](#六资源占用)      |
...
| 场景                   | iptables         | Cilium Native    | Cilium Overlay   |
| ---------------------- | ---------------- | ---------------- | ---------------- |
| Pod-to-Pod c64 长连接  | 110,910 req/s    | 105,378 req/s    | 100,836 req/s    |
| Via Svc c64 长连接     | 110,838 req/s    | 105,070 req/s    | 100,084 req/s    |
| Via Svc c256 长连接    | 114,704 req/s    | 107,692 req/s    | 102,554 req/s    |
| **Via Svc c64 短连接** | **30,413** req/s | **13,786** req/s | **14,263** req/s |

### 长连接：iptables 领先 5-11%
...
短连接基线 iptables（30,413）是 Cilium（~14,000）的 **2.2 倍**，远大于长连接的差距。根因在于**长短连接命中的代码路径完全不同**：
...
值得专门指出：本次测试中 Cilium 的 TCP_RR（长连接请求-响应延迟）p99 为 95-96 µs，**低于** iptables 的 111 µs。
...
| Service 数     | iptables              | Cilium Native        | Cilium Overlay       |
| -------------- | --------------------- | -------------------- | -------------------- |
| 基线（小规模） | 30,413 req/s          | 13,786 req/s         | 14,263 req/s         |
| 5000           | 21,740（-28.5%）      | 12,473（-9.5%）      | 13,149（-7.8%）      |
| 10000          | 17,558（**-42.3%**）  | 11,963（**-13.2%**） | 12,618（**-11.5%**） |
| 规则数/LB 条目 | 210,058 → **419,681** | 60,006 → 119,832     | 60,030 → 119,888     |

:::tip[O(n) vs O(1)：10 Endpoint 把差距放大到肉眼可见]

短连接是真正的考验——每个新连接的 SYN 包都要重做 Service 选择，无法命中 conntrack 缓存。

- **iptables 是 O(n) 顺序遍历**：每个 SYN 包顺序匹配 KUBE-SERVICES → KUBE-SVC-XXX → KUBE-SEP-XXX 规则链。**每个 Endpoint 多一条 KUBE-SEP 规则**，所以 10000 svc × 10 ep 时规则数高达 **42 万条**。短连接退化从 5000 svc 的 -28.5% 恶化到 10000 svc 的 -42.3%，随规则数近乎线性崩塌。
...
**与单 Endpoint 测试的对比**：早前每 svc 只挂 1 个 Endpoint 时，iptables 10000 svc 短连接退化是 -37%、规则数 6 万。换成 10 Endpoint 后退化加深到 -42%、规则数飙到 42 万——**Endpoint 越多，iptables 的 O(n) 劣势越严重**，这才贴近真实业务（真实 Service 普遍多副本）。

:::

### 那么 Cilium 何时反超 iptables？

注意绝对值：**即使到 10000 svc，iptables 短连接（17,558）仍高于 Cilium（~12,300）**。但领先优势从基线的 2.1 倍已收窄到 1.43 倍。
...
一个反直觉的结果：满载（10000 svc × 10 ep）下 **kube-proxy 内存（278 MB）反而高于 Cilium Agent（153-165 MB）**，CPU（112m）也与 Cilium Agent 相当。

原因是 kube-proxy 要在用户态维护那 **42 万条 iptables 规则**的完整内存表示，并在每次 Service/Endpoint 变更时做规则 diff 与全量刷新——规则越多，kube-proxy 内存和 CPU 越高。这也解释了为什么 iptables 模式的 kube-proxy 内存从早前单 Endpoint（~31 MB，6 万规则）暴涨到现在的 278 MB（42 万规则）。
...
| Service 数量大（万级）、有大量短连接业务             | **Cilium**：iptables 短连接随规则数线性崩塌（42 万规则 -42%），Cilium 稳定 |
| 需要 NetworkPolicy / Hubble 可观测性 / Identity 安全 | **Cilium**：L3/L4 策略与 Hubble 零开销，iptables 不具备                    |
| 只跑长连接业务（连接池 / keepalive）                 | 都行：长连接对规模和方案都不敏感                                           |

核心权衡：**换 Cilium 在小规模极限压测下损失 ~5-11% 长连接 RPS（真实负载无感），换来大规模下不崩塌的短连接性能 + 零开销的安全与可观测能力。** 对中大规模或有安全合规需求的集群，这笔交易划算。
