...
本文与以下两篇构成 Native Routing 模式的设计原理三部曲：

1. 本文：解释 Native 模式为何只能走 legacy host routing，以及 Overlay 模式如何拿到 BPF host routing。
2. **[为什么 Native 要加 local-router-ipv4](./local-router-ipv4.md)**：理解 legacy host routing 下的 `cilium_host` IP 配置需求。
3. **[为什么 Native 禁用 sysctlfix](./sysctlfix.md)**：理解同一模式下 `systemd-sysctl` 重启对 rp_filter 的连带影响。
