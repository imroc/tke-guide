...
sysctlfix 会在每次 cilium pod 启动时**重启宿主机的 `systemd-sysctl.service`**，全量重放发行版 sysctl 默认值。对 Native 模式这会直接断网：`tke-eni-agent` 为配合 VPC-CNI 策略路由把 eth0 的 `rp_filter` 设为 `2`（loose），重放把它打回 `1`（strict）后，辅助网卡 IP 的非对称流量被严格反向路径校验丢弃。

Native 模式**禁用 sysctlfix 即可，无需额外补偿**：`endpointRoutes` 为每个 Pod 提供对称的 per-endpoint 路由，lxc 接口的 rp_filter 值不影响连通性（Overlay 模式则不同——路由聚合到 `cilium_host` 导致非对称，需要部署 `cilium-sysctl-override` DaemonSet 补偿）。

完整机制分析（sysctlfix 的两大动作、udev 对新接口的 sysctl 重放、路由对称性原理、两种模式的决策表与故障排查）见 [sysctlfix 与 rp_filter 机制详解](./sysctlfix.md)。
...
- [sysctlfix 与 rp_filter 机制详解](./sysctlfix.md)
