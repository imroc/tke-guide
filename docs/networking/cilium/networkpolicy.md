...
下面用法实践中的部分能力依赖 cilium 的 **L7 DNS Proxy**（具体包括 `toFQDNs` 和 `toPorts.rules.dns` 这两类规则），cilium 通过 BPF + iptables TPROXY 把被策略选中的 Pod 发出的 DNS 查询重定向到 cilium-agent 内置的 DNS 代理。

本系列教程提供的 3 种推荐部署方案——**Native Routing (VPC-CNI)**、**Overlay (VPC-CNI)**、**Overlay (GR)**——都完整支持 `toFQDNs` 和 `rules.dns`，下文所有示例可直接套用。

:::note[GR Native Routing 不可用]

第 4 种组合 GR + Native Routing 因兼容性问题已不再作为推荐方案（除 L7/DNS 不支持外，跨节点 Pod-to-Pod 流量也不通），详见 [为什么不提供 GR Native Routing 部署方案？](./appendix/gr-native-not-recommended.md)。GR 集群希望使用 cilium，请改用 **Overlay 模式**。
