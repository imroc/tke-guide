...
下面用法实践中的部分能力依赖 cilium 的 **L7 DNS Proxy**（具体包括 `toFQDNs` 和 `toPorts.rules.dns` 这两类规则）。这些能力的工作原理是：cilium 通过 BPF + iptables TPROXY 把被策略选中的 Pod 发出的 DNS 查询重定向到 cilium-agent 内置的 DNS 代理，由代理解析、记录响应、并把响应中的 IP 加入到 toFQDNs 的允许列表。

**这条链路在不同 cilium 网络模式下的表现不同**：

| 模式                     | 是否支持 toFQDNs / dns L7 | 原因                                                                                                                                                                                                     |
| ------------------------ | ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Native Routing (VPC-CNI) | ✅ 完整支持               | Pod 直接挂在弹性网卡上，不走 Linux bridge，DNS 重定向链路完整                                                                                                                                            |
| Native Routing (GR)      | ❌ 不支持                 | 与 tke-bridge [generic-veth chaining](https://docs.cilium.io/en/stable/installation/cni-chaining-generic-veth/) 共存，包经过 cbr0 桥转发时绕过 socket dispatch，DNS 重定向到 cilium DNS proxy 收不到流量 |
| Overlay (VPC-CNI / GR)   | ✅ 完整支持               | cilium 完全接管 Pod datapath，DNS 重定向链路完整                                                                                                                                                         |

**症状**：在 GR Native Routing 模式下，被含 `rules.dns` 或 `toFQDNs` 的 CNP 选中的 Pod，**所有 DNS 查询都会超时**（不是返回 NXDOMAIN/REFUSED，而是无任何响应），即使是集群内服务名（如 `kubernetes.default.svc`）也无法解析。

**官方背书**：cilium 官方文档已将 "Layer 7 Policy" 列为 generic-veth chaining 模式的已知 Limitations，详见 [Cilium Docs - Generic Veth Chaining § Limitations](https://docs.cilium.io/en/stable/installation/cni-chaining-generic-veth/#limitations) 及跟踪 issue [cilium/cilium#12454](https://github.com/cilium/cilium/issues/12454)。

**推荐做法**：

- **VPC-CNI Native Routing / Overlay**：可放心使用 `toFQDNs` 和 `rules.dns`，下文所有示例都可直接套用
- **GR Native Routing**：避免在 NetworkPolicy 中使用 `rules.dns` 和 `toFQDNs`，改用以下替代方案：
  - 用 `toCIDR` / `toCIDRSet` 列出目标网段（如腾讯云 API 的网段）
  - 用 `toEntities: [world]` 允许所有公网（粒度较粗）
  - 用 `toEndpoints` 配合命名空间/Pod 标签做集群内访问控制
  - 如必须按域名做出向控制，请改用 Overlay 模式

下文示例中凡涉及 `rules.dns` 或 `toFQDNs` 的，会在示例标题旁标注 **GR Native Routing 不可用**。
...
:::warning[GR Native Routing 不可用]

此策略包含 `rules.dns`，在 **GR Native Routing** 模式下被选中的 Pod 会出现 DNS 解析超时，详见 [模式兼容性](#模式兼容性)。VPC-CNI Native Routing 和 Overlay 模式均可正常使用。

GR 集群下若要做"默认拒绝 egress + 允许 DNS"，请改用不带 `rules.dns` 的 L4 写法（去掉 `rules: dns: ...` 那几行，仅保留 `port: "53" protocol: ANY`），代价是失去 toFQDNs 能力。
...
此示例使用了 `rules.dns` + `toFQDNs`，在 **GR Native Routing** 模式下不可用。GR 集群请用 `toCIDR` 列出腾讯云 API 的网段（如 `9.0.0.0/8`、`169.254.0.0/16` 等）替代 `toFQDNs` 部分，并删除 DNS L7 规则。VPC-CNI Native Routing 和 Overlay 模式可直接使用。详见 [模式兼容性](#模式兼容性)。
...
此示例使用 `rules.dns` + `toFQDNs`，在 **GR Native Routing** 模式下被选中的 Pod 会出现 DNS 解析超时。GR 集群请改用 `toCIDR` 列网段，或切到 Overlay 模式。VPC-CNI Native Routing 和 Overlay 模式可直接使用。详见 [模式兼容性](#模式兼容性)。
