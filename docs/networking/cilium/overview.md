...
| **E2E 测试**    | 功能测试方法与实测数据            | 所有安装后的用户 |
| **性能测试**    | 网络基准性能与各方案横向对比      | 关注性能的用户   |
...
| **已验证的节点操作系统**                 | 7 种 OS 的兼容性验证结果                  |
| **Cilium Host Routing**                  | Legacy vs BPF 的机制、命中条件、横向对比  |
| **为什么 Native 要加 local-router-ipv4** | 原理与地址选择                            |
| **为什么 Native 禁用 sysctlfix**         | rp_filter 差异与决策逻辑                  |
